...
int calculate_block(int virtual_memory_limit, int cpu_time_limit, int start_column, int end_column, int rows_a, int columns_b, int rows_b, int matrixA[][rows_b], int matrixB[][columns_b], char* result_file, char* my_file){
    ...
    long int memoryLimitBytes = 1000000 * virtual_memory_limit;

    // Set hard limits for cpu-time 
    struct rlimit cpuRL;
    getrlimit (RLIMIT_CPU, &cpuRL);

    cpuRL.rlim_max = cpu_time_limit;
...
int main(int argc, char** argv){
    ...
    if(argc == 7){
        ...
        int virtual_memory_limit = atoi(argv[6]);
        ...
        if(CHILD){
            ...
            for(int i = 0; i <= columns / (number_of_columns_per_process*number_of_children); i++){
                ...
                nom += calculate_block(virtual_memory_limit, cpu_time_limit, number_of_children * number_of_columns_per_process * i + my_number * number_of_columns_per_process,
        ...
        if(PARENT){
            ...
            struct rusage* usage = calloc(1, sizeof(struct rusage));
            getrusage(RUSAGE_SELF, usage);
            printf("I'm a child with pid(%d) and number(%d) and I did %d multiplications\nI used -> \nuser_time: %lds and %ldms\nsys_time: %lds and %ldms\n"
            ,getpid(), getpid()-main_pid, nom, usage->ru_utime.tv_sec, usage->ru_utime.tv_usec, usage->ru_stime.tv_sec, usage->ru_stime.tv_usec);
